...
void mc_line(float *target, plan_line_data_t *pl_data)
{
  ...
  if (plan_buffer_line(target, pl_data) == PLAN_EMPTY_BLOCK) {
    if (bit_istrue(settings.flags,BITFLAG_LASER_MODE)) {
      // Correctly set gripper state, if there is a coincident position passed. Forces a buffer
      // sync while in M3 laser mode only.
      if (pl_data->condition & PL_COND_FLAG_GRIPPER_CW) {
        gripper_sync(PL_COND_FLAG_GRIPPER_CW, pl_data->gripper_speed);
      }
    }
  }
}
...
void mc_homing_cycle(uint8_t cycle_mask)
{
  ...
  // TODO: Move the pin-specific LIMIT_PIN call to limits.c as a function.
    ...
    if (limits_get_state()) {
      mc_reset(); // Issue system reset and ensure gripper and coolant are shutdown.
